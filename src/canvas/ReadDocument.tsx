...
import { DocumentCanvas, type CanvasActions } from './DocumentCanvas'

const noop = () => {}

// The same canvas and layout normalization as reading in the app, without persistence.
export function ReadDocument({ initial, zoomHost = null, minimap = false, minimapSize = 'proportional', onReady = noop, staticWidgets = false, initialScale, onActions = noop }: {
  ...
  return <HistoryContext value={history}><DocumentCanvas doc={doc!} editable={false} zoomHost={zoomHost}
    minimap={minimap} minimapSize={minimapSize} selectedIds={[]} tool={null} staticWidgets={staticWidgets} initialScale={initialScale}
    onSelect={noop} onToolChange={noop} onActions={onActions} onActive={noop} onMainReady={onReady}
