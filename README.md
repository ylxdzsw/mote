...
Resize previews use transient editor decorations; only the released height is saved, and Escape/pointer cancellation restores the previous layout. Each committed boundary resize is one main-editor undo step. Automatic release is a layout normalization, not a separate undo step, so shrinking text after a release does not resurrect the old minimum.

When an anchor is deleted, its floating objects move to the closest surviving preceding block, or the first remaining block. Their content is retained. Undo/redo currently covers text, table structure, spacers, and main-text segment resizing within each editor, **not** theme, floating geometry, image replacement, reset, or object creation/deletion. V0 has no compatibility or migration promises yet.
